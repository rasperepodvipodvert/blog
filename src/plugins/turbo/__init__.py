...
XML_URL = """
        <item turbo="true">
            ...
            <metrics>
                <yandex schema_identifier="64995766">
                </yandex>
            </metrics>
...
class SitemapGenerator(object):
        ...
        if self.format == 'xml':
            ...
            for regstr in self.sitemapExclude:
                if re.match(regstr, pageurl):
                    ...
                    break
            if not flag:
                fd.write(XML_URL.format(self.siteurl, pageurl, lastmod, chfreq, pri, content))
        else:
    ...
    def generate_output(self, writer):
        path = os.path.join(self.output_path, 'turbo.{0}'.format(self.format))
